...
\begin{proof}
	...
	Давайте сначала перетащим формы $\d x$, $\d y$, $\d z$ из $S$ (тут живут переменные $x$, $y$, $z$) в $G$ (тут живут переменные $u$, $v$).
	Для этого надо посчитать дифференциал отображения $\vec r$:
	\[
		...
		\end{pmatrix}
	\]
	А потом перетащить по определению:
	\begin{align*}
		r^* \d x\left(\begin{pmatrix} u \\ v\end{pmatrix}\right)
		 = \d x\left(\d \vec r\begin{pmatrix} u \\ v\end{pmatrix}\right)
		&= \d x\left(\begin{pmatrix}x_u' u + x_v' v \\ y_u' v + y_v' v \\ z_u' u + z_v' v \end{pmatrix}\right)
		 = x_u' u + x_v' v \\
		r^* \d x &= x_u' \d u + x_v' \d v \\
		r^* \d y &= y_u' \d u + y_v' \d v \\
		r^* \d z &= z_u' \d u + z_v' \d v
	\end{align*}
	Также можем перетащить произведения:
	\begin{align*}
		r^* (\d x \wedge \d y)
		&= (r^* \d x) \wedge (r^* \d y) = (x_u' \d u + x_v' \d v) \wedge (y_u' \d u + y_v' \d v) = \\
		&= x_u'y_u'\underbrace{\d u \wedge \d u}_{0}
		+ x_u'y_v'\d u \wedge \d v
		+ x_v'y_u'\underbrace{\d v \wedge \d u}_{-\d u \wedge \d v}
		+ x_v'y_v'\underbrace{\d v \wedge \d v}_{0}
		= \\
		&= (x_u'y_v' - x_v'y_u') (\d u \wedge \d v)
	\end{align*}
	А теперь доказываем теорему:
	\begin{gather*}
		...
		= \int\limits_G P(y_u' z_v' - y_v' z_u') + Q(z_u' x_v' - z_v' x_u') + R(x_u' y_v' - x_v' y_u') \d u \d v = \\
		= \int\limits_G \left(P\underbrace{(y_u' z_v' - y_v' z_u')}_{\d y \wedge \d x} + Q\underbrace{(z_u' x_v' - z_v' x_u')}_{\d z \wedge \d x} + R\underbrace{(x_u' y_v' - x_v' y_u')}_{\d x \wedge \d y} \right) (\d u \wedge \d v) = \\
		\begin{aligned}
		\end{aligned} \\
