...
\begin{proof}
	...
	\[
		\alpha(P)
		= \alpha\left(\tildes1P\right) + \alpha\left(\tildes2P\right)
		+ \alpha\left(\tildes3P\right) + \alpha\left(\tildes4P\right)
	\]
	Не умаляя общности,
	...
	Последовательность этих вложенных прямоугольников стремится к нулю по размеру.
	Выберем общую точку $z_0$.
	\begin{align*}
		f(z) &= f(z_0) + k(z - z_0) + \beta(z) \quad \beta(z-z_0) = o(z - z_0) \\
		\int\limits_{P_n} f(z) \d z
		&= \underbrace{\int\limits_{P_n} f(z_0) \d z}_{=0} + \underbrace{\int\limits_{P_n} k(z-z_0) \d z}_{=0}
		+ \int\limits_{P_n} \beta(z) \d z
	\end{align*}
	Оценим
	\begin{gather*}
