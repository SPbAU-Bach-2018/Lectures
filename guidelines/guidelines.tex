...
\item
	...
	\begin{center}\begin{tabular}{|c|c|c|}
		...
		\hline \verb!$o(||h||^2)$! & $o(||h||^2)$ \bad \\
		\hline \verb!$o(\|h\|^2)$! & $o(\|h\|^2)$ \ok \\
		\hline
	\end{tabular}\end{center}
...
\end{verbatim}
	...
	\item
		Набор длинных стрелок с подписями идёт с помощью комманд \verb|\xlongrightarrow| и аналогичных:
		\begin{center}\begin{tabular}{|c|c|c|}
			\hline Это... & ...верстается так & \\
			\hline \verb'$\stackrel{ab=1}{\Ra}$' & $\stackrel{ab=1}{\Ra}$ \bad \\
			\hline \verb'$\xLongrightarrow{ab=1}$' & $\xLongrightarrow{ab=1}$ \ok \\
			\hline \verb'$\xLongrightarrow[cd=3]{ab=1}$' & $\xLongrightarrow[cd=3]{ab=1}$ \ok \\
			\hline \verb'$\xrightarrow{f}$' & $\xrightarrow{f}$ \ok \\
			\hline
		\end{tabular}\end{center}

	\item
		Сложные индексы (перед выражением или если их много) набираем с помощью \verb'\tensor':
		\begin{center}\begin{tabular}{|c|c|c|}
			\hline Это... & ...верстается так & \\
			\hline \verb'$a {}_R \mid b$' & $a {}_R \mid b$ \bad \\
			\hline \verb'$a \tensor[_R]{\mid}{} b$' & $a \tensor[_R]{\mid}{} b$ \ok \\
			\hline \verb'${}_1^2_3^4 X_5^6_7^8$' & \bad \\
			\hline \verb'$\tensor[_1^2_3^4]{X}{_5^6_7^8}$' & $\tensor[_1^2_3^4]{X}{_5^6_7^8}$ \ok \\
			\hline \verb'$\tensor*{X}{*_5^{66}_{77}^8}$' & $\tensor*{X}{*_5^{66}_{77}^8}$ \ok \\
			\hline
		\end{tabular}\end{center}
...
\item
	...
	\begin{enumerate}
	\item
		Набор кода в строке (название классов, методов, операторы) используется аналог команды \verb'\verb':
		\verb'\cpp' для плюсов, \verb'\java' для Java и так далее.
		\begin{center}\begin{tabular}{|c|c|c|}
			\hline Это... & ...верстается так & \\
			\hline \verb'std::sort<int>'        & std::sort<int>        \bad \\
			\hline \verb"\verb'std::sort<int>'" & \verb'std::sort<int>' \bad \\
			\hline \verb"\cpp'std::sort<int>'"  & \cpp'std::sort<int>'  \ok  \\
			\hline
		\end{tabular}\end{center}

	\item
		Набор кусков кода уже делаем в аналоге окружения \verb'verbatim':
		\verb'cppcode' для плюсов, \verb'javacode' для Java и так далее:
		\begin{center}\begin{tabular}{|c|c|c|}
			\hline Это... & ...верстается так & \\
...
\item
	...
	\begin{enumerate}
	\item
		Разделители, как \verb'\section', \verb'\chapter' и прочие не порождают отступа,
		ограждаются одной пустой строкой с каждой стороны.

	\item
		Все окружения порождают отступ внутри себя.
...
\end{verbatim}
	...
	\item
		Если вы используете короткие строки в \verb'\item',
		то пишите в одну строку и отступите перед \verb'\item', пустых строк не нужно.
		Иначе \verb'\item' не сдвигается относительно \verb'enumerate', \verb'itemize' и \verb'description',
		содержимое пишется со следующей строки со сдвигом, а перед следующим \verb'item'-ом ставится пустая строка:
\begin{verbatim}
\begin{itemize}
    \item 1
    \item 2
