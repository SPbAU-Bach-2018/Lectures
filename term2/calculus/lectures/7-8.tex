...
\[||A|| \le (\sum_{i = 1}^n \sum_{j = 1}^m a_{i, j}^2)^{\frac{1}{2}}\]
\begin{proof}
$x = x_1e_1 + \dots + x_n e_n$ \\
$||x||^2 = x_1^2 + \dots + x_n^2$ \\
$||A(x)|| = ||A(x_1e_1 + \dots + x_n e_n)|| = ||x_1A(e_1) + \dots + x_nA(e_n)|| \le |x_1| \cdot ||A(e_1)|| + \dots + |x_n| \dot ||A(e_n)||$ \\
...
$||A(x)|| \le ||x||(\sum_{i = 1}^n \sum_{j = 1}^m a_{i, j}^2)^{\frac{1}{2}} \le (\sum_{i = 1}^n \sum_{j = 1}^m a_{i, j}^2)^{\frac{1}{2}}$ при $||x|| \le 1$\\
$\sup_{||x|| \le 1}||A(x)|| \le (\sum_{i = 1}^n \sum_{j = 1}^m a_{i, j}^2)^{\frac{1}{2}}$
\end{proof}
\begin{Def}
